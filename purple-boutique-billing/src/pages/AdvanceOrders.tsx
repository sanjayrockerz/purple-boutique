...
export default function AdvanceOrders({ onOrderCompleted }: AdvanceOrdersProps) {
  ...
  const create = async (event: FormEvent) => {
    ...
    if (!Number.isFinite(total) || total <= 0 || !Number.isFinite(deposit) || deposit <= 0 || deposit >= total) { setError('Deposit must be greater than â‚¹0 and less than the total order amount.'); setSaving(false); return }
    try {
      const created = await createAdvanceOrder({ ...form, totalAmount: total, depositAmount: deposit, createdByName: user?.name || user?.email || 'Staff' })
  ...
  const invoiceFile = (order: AdvanceOrder) => invoicePdfFile({ invoiceNo: order.invoice_number || order.deposit_id, date: order.completed_at || new Date().toISOString(), customerName: order.customer_name, phone: order.phone, address: order.address, items: [{ name: order.product_name, quantity: 1, base_price: order.total_amount, line_total: order.total_amount, unit: 'piece' }], subtotal: order.total_amount, shipping: 0, total: order.total_amount, paymentMode: order.final_payment_method || 'Paid' })
  const printFinal = (order: AdvanceOrder) => printThermalReceipt({ invoiceNo: order.invoice_number || order.deposit_id, date: order.completed_at || new Date().toISOString(), customerName: order.customer_name, phone: order.phone, items: [{ name: order.product_name, qty: 1, price: order.total_amount, line_total: order.total_amount }], subtotal: order.total_amount, shipping: 0, total: order.total_amount })
  const whatsappInvoice = (order: AdvanceOrder) => {
    const message = buildProfessionalWhatsAppMessage({ customerName: order.customer_name, phone: order.phone, invoiceNumber: order.invoice_number || order.deposit_id, invoiceDate: order.completed_at || undefined, invoiceUrl: publicInvoiceUrl(order.invoice_number || order.deposit_id), paymentMode: order.final_payment_method || 'Paid', items: [{ name: order.product_name, qty: 1, unit: 'piece', unitType: 'unit', rate: order.total_amount, lineTotal: order.total_amount }], subtotal: order.total_amount, total: order.total_amount })
